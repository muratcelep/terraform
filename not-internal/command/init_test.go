...
import (
	...
	"github.com/muratcelep/terraform/not-internal/providercache"
...
// https://github.com/muratcelep/terraform/issues/518
...
// Verify that plugin-dir doesn't prevent discovery of not-internal providers
func TestInit_pluginDirWithBuiltIn(t *testing.T) {
	td := tempDir(t)
	testCopyDir(t, testFixturePath("init-not-internal"), td)
	defer os.RemoveAll(td)
	defer testChdir(t, td)()
...
func TestInit_invalidBuiltInProviders(t *testing.T) {
	...
	//   not exist at all.
	td := tempDir(t)
	testCopyDir(t, testFixturePath("init-not-internal-invalid"), td)
	defer os.RemoveAll(td)
	defer testChdir(t, td)()
